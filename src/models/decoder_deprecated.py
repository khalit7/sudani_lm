"""DEPRECATED — the original hand-written decoder, superseded by src/models/decoder.py.

Kept for two reasons: it is the learning artifact this project was built to write, and it is the
correctness oracle for the new implementation. tests/test_decoder.py asserts that the new
SDPA attention reproduces this module's output exactly.

Note that MaskedMultiHeadAttn computes `K @ Qᵀ` rather than the conventional `Q @ Kᵀ`. That is
*not* a bug: it is standard attention with the q and k projections swapped, and the model simply
learns the swap. To compare the two implementations, swap q_proj and k_proj weights.

Do not use for new training runs: post-LN, additive sinusoidal positions, no weight tying, no
final norm before the LM head, and an O(seq_len²) score matrix that is 64x slower and 2.8x
heavier than F.scaled_dot_product_attention at Stage-B shapes.
"""
