...
- Private, non-redistributed use only. Nothing scraped is republished; `data/` is gitignored.
...
| **alnilin.com** (comments) | news comments | **561,752 comments** total; crawl running (~50%+ fetched) | **very high** | easy | **partial** | Open WordPress REST (`/wp-json/wp/v2/comments`), clean JSON, `X-WP-Total` gives exact scale. Reader comments are where dialect lives. |
| **alnilin.com** (articles) | news | ~550K articles (sitemap depth) | low (silver) | easy | **partial** | Same wp-json route, `posts` endpoint — runs after comments in the same crawl. Sudan-topical MSA, ranked down by the classifier. |
| **anasudani.net/forum** | forum (phpBB) | **1,161,442 posts / 52,973 topics / 26,496 members**; frozen ~Jan 2017 | high | easy | **queued** | Highest-value un-scraped forum. **No robots.txt at all** (404 → unrestricted). Crawl via `viewforum.php` listings, not `?t=` ID sweep — IDs are sparse across 40k–66k. |
| **sudanile.com** | news + opinion | **142,731 posts** (`X-WP-Total`) | low–med | easy | **queued** | Open wp-json full-content JSON; 189 Wayback page-blocks. **0 comments**. Biggest low-effort silver haul; columnists drift into dialect. |
| **wadmadani.com/vb** | forum (vBulletin) | thread IDs ≥ **41,443** | high | moderate | **queued** | `robots.txt` allows all but sets **`crawl-delay: 60`** — obey it. Host is failing (repeated **507/508 Insufficient Storage**); use the light `/vb/archive/index.php/t-N.html` pages and backfill gaps from Wayback (55 CDX page-blocks). |
| **sudanyat.org/vb** (سودانيات) | dead forum | thread IDs ≥ **39,234**; **43** Wayback page-blocks | high | moderate | **queued** | Live site serves only "المنتدى تحت الصيانة"; all `/vb/` paths 302 to it. Mine Wayback `archive/index.php/t-N.html`. Contains a multi-page *مصطلحات عامّية سودانية* thread. |
| **mugrn.net/vb** (المقرن) | dead forum | now Sedo-parked; hosts "موسوعة الشعر الحلمنتيشي" | high | moderate | **queued** | Wayback only. |
| **algorer.net/vb** (القـريـر) | dead forum | thread IDs ≥ **20,071**; DNS dead | high | moderate | **queued** | Wayback only; indexed threads include أمثال/سيرة سودانية collections. |
...
| **Blogger cluster** — hageebatalfun (كلمات الحقيبة), sudaneseshortstorieswriters, sudanese-novels, katabsudsnese | lyrics / stories | small–mid, all reachable | **high** | easy | **queued** | Use the Blogger Atom feed `/feeds/posts/default?max-results=500&start-index=N` — full post bodies, no HTML fragility. |
| **aghaniwamthal.com** | proverbs + lyrics | small (136 KB home) | **high, very clean** | easy | **queued** | Token-for-token the densest dialect found; tiny but cheap. |
